...
import com.lw.export_demo.excel.core.converter.ReadConverterContext;
...
public class DefaultExcelReader<T> {
    ...
    private static final Log log = LogFactory.getLog(DefaultExcelReader.class);
...
//   补充     log.info("FirstRowNum:{},LastRowNum:{}", firstRowNum, lastRowNum);
        if (lastRowNum < 0) {
//   补充         log.info("Reading excel takes {} milliseconds", System.currentTimeMillis() - startTime);
            return Collections.emptyList();
        }
...
//     补充           log.info("Row of {} is null,it will be ignored.", i);
                continue;
            }
            boolean noMatchResult = rowFilter.negate().test(row);
            if (noMatchResult) {
//    补充            log.info("Row of {} does not meet the filtering criteria, it will be ignored.", i);
                continue;
            }
